...
int step_M_D__check__sort_s4(Sink sink__V1, Term term__V2)
{
  ...
  do {
    ...
    Term sub__V5 = SUB(term__V2, 2); int sub__V5_count = term__V2_count*LINK_COUNT(sub__V5); permitUnusedInt(sub__V5_count);
    ...
    { START(sink__V1, _M__sTextNil); END(sink__V1, _M__sTextNil); } UNLINK_NamedPropertyLink(sink__V1->context, namedP__V6); 
...
int step_M_D__check__sort_s2(Sink sink__V8, Term term__V9)
{
  ...
  do {
    ...
    /* sub__V10 = &#31 */
    Term sub__V11 = SUB(term__V9, 1); int sub__V11_count = term__V9_count*LINK_COUNT(sub__V11); permitUnusedInt(sub__V11_count);
    /* sub__V11 = &#32 */
    ...
    { int test__V16;
      FORCE(sink__V8->context, sub__V10); LINK(sink__V8->context, sub__V10);
      test__V16 = (int) (strstr(SYMBOL(sub__V10), "$" ) != NULL);
      UNLINK(sink__V8->context, sub__V10);  
      if (test__V16) 
      {
         UNLINK(sink__V8->context, sub__V10);  UNLINK(sink__V8->context, sub__V12);  UNLINK(sink__V8->context, sub__V13); { START(sink__V8, _M__sTextNil); END(sink__V8, _M__sTextNil); }
    ...
    UNLINK_NamedPropertyLink(sink__V8->context, namedP__V14); 
    UNLINK_VariablePropertyLink(sink__V8->context, varP__V15);
    return 1;
  } while (0);
  return 0;
}
...
int step_M_D__check__sort_s3(Sink sink__V17, Term term__V18)
{
  ...
  do {
    ...
    Term sub__V19 = SUB(term__V18, 0); int sub__V19_count = term__V18_count*LINK_COUNT(sub__V19); permitUnusedInt(sub__V19_count);
    CRSX_CHECK_SORT(sink__V17->context, sub__V19, &sort_M_Reified_xSort); /* sub__V19 = &#31 */
    Term sub__V20 = SUB(term__V18, 1); int sub__V20_count = term__V18_count*LINK_COUNT(sub__V20); permitUnusedInt(sub__V20_count);
    ...
    UNLINK_NamedPropertyLink(sink__V17->context, namedP__V24); 
    UNLINK_VariablePropertyLink(sink__V17->context, varP__V25);
    return 1;
  } while (0);
  return 0;
}
...
int step_M_D__check__sort_s1(Sink sink__V26, Term term__V27)
{
  ...
  switch (choice__V29)
  {
    case Data_M_SORT_PROPERTY: { /* Function CDEFS-D-check-sort-1$D-check-sort$1$SORT-PROPERTY case SORT-PROPERTY */
      ...
      UNLINK_NamedPropertyLink(sink__V26->context, namedP__V36); 
      UNLINK_VariablePropertyLink(sink__V26->context, varP__V37);
      
      return 1;
    break; } case Data_M_SORT_VARIABLE: { /* Function CDEFS-D-check-sort-2$D-check-sort$1$SORT-VARIABLE case SORT-VARIABLE */
      ASSERT(sink__V26->context, !strcmp(SYMBOL(term__V27),  "D-check-sort$1" ));
      Term sub__V38 = SUB(term__V27, 0); int sub__V38_count = term__V27_count*LINK_COUNT(sub__V38); permitUnusedInt(sub__V38_count);
      ...
      UNLINK_NamedPropertyLink(sink__V26->context, namedP__V42); 
      UNLINK_VariablePropertyLink(sink__V26->context, varP__V43);
      
      return 1;
    break; } case Data_M_SORT: { /* Function CDEFS-D-check-sort-3$D-check-sort$1$SORT case SORT */
      ASSERT(sink__V26->context, !strcmp(SYMBOL(term__V27),  "D-check-sort$1" ));
      Term sub__V44 = SUB(term__V27, 0); int sub__V44_count = term__V27_count*LINK_COUNT(sub__V44); permitUnusedInt(sub__V44_count);
      ...
      UNLINK_NamedPropertyLink(sink__V26->context, namedP__V49); 
      UNLINK_VariablePropertyLink(sink__V26->context, varP__V50);
